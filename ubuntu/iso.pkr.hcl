...
source "vsphere-iso" "ubuntu_base" {
...
build {
  sources = ["source.vsphere-iso.ubuntu_base"]
  ...
  provisioner "shell" {
      ...
      execute_command = "echo ${var.ssh.password} | sudo -S sh -c '{{ .Vars }} {{ .Path }}'"
  }
  provisioner "shell" {
      script = "scripts/install-ad.sh"
